...
- **Search** by session **or player** name, an **"open spots only"** toggle, an **"◯ empty sessions"** toggle (court times nobody has booked yet — a fresh court), and a **Women's** filter — show all, **only** ladies' sessions (Ladies Night, Ladies Matchups, …), or **hide** them.
...
- **"That's me"** — tell the app who you are (search your name, no login) and it flags what you're **registered** for ("✓ You're in"), with a "My sessions" filter. A session card always states your status up front. Opening a session live-refreshes its roster so your status is current — and if you tap **Book on PodPlay** and come back to this tab afterward, it automatically re-checks that same session so your status updates without reopening the card.
- **Waitlist tracking** — PodPlay doesn't publish waitlist membership to anonymous visitors, so by default a full session's card offers **"I'm waitlisted — track it"**. Tracked sessions show "◷" everywhere (calendar days show **✓ and ◷ side by side**, rows get the amber badge, "My sessions" includes them), the mark is stored on your device, and it clears itself automatically once the roster shows you got in.
- **Optional PodPlay login** — log in with your PodPlay email/password (in **You & friends**) and live data is fetched **as you**: member-visible waitlist entries light up automatically, and your identity is set exactly (your sign-in ID *is* your roster ID — no name search). The password goes only to PodPlay's sign-in service (Google Identity, the same endpoint PodPlay's own app uses) and is never stored — this device keeps just a session token, and logging out drops it. Google/Apple-SSO PodPlay accounts can't log in this way.
